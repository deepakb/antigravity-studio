...
> **Nexus Studio** — Enterprise AI Orchestration Engine for **@nexus/web**
...
| `vite.config`, `rollupOptions`, `manualChunks`, `import.meta.env`, `HMR`, `vite-plugin-`, `build.lib` | `vite-specialist` | `react-performance-guru` |
| `redesign`, `upgrade UI`, `ambient background`, `hero section`, `glassmorphism`, `glow effect` | `creative-director` + `ui-design-engineer` | use `/redesign` workflow |
...
| `Angular`, `NgRx`, `standalone component`, `inject()`, `Signal`, `@Component` | `angular-specialist` | `enterprise-architect`, `accessibility-auditor` |
| `NestJS`, `@Module`, `@Injectable`, `@Controller`, `Guard`, `Interceptor`, `Pipe` | `nestjs-specialist` | `backend-specialist`, `security-engineer` |
| `Svelte`, `SvelteKit`, `+page`, `+layout`, `$props`, `$state`, `$derived`, `rune` | `svelte-specialist` | `frontend-specialist`, `qa-engineer` |
...
|-------|---------|----------------|------|
| Simple | 1 file, 1 domain | Inline 🧠 THINK (1–3 sentences) | None — proceed |
| Compound | 2–5 files, 1–2 domains | 🧠 THINK + 📋 PLAN | Soft — user may redirect |
...
```
🧠 THINK
  What's really being asked: [restate actual goal]
...
- `@vite-specialist` owns all `vite.config.ts` / build / plugin / chunk questions
- ⛔ `@nextjs-expert` **never** activates on this profile — see hard block in AGENTS.md
...
- Passport JWT guards on all protected routes
- `@nestjs/swagger` decorators on every DTO and controller
- `/health` endpoint with `@nestjs/terminus` always present
...
**`angular-enterprise`**:
- Standalone components, `inject()` DI pattern throughout
...
- Content Collections API for all markdown/MDX content
...
**Managed by**: Nexus Studio v2026-04-10
